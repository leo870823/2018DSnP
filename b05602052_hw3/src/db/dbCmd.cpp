...
if(options.size()==2)
 {       
       if( isValidVarName(options[0]))  //check_key
      {  
         
         int buffer=0;
          if(myStr2Int(options[1],buffer))    //check_value
           {   int pos=dbjson.size();
                for(int i=0;i<dbjson.size();i++)
               if(options[0]==dbjson[i].key()) 
               {
                 cerr<<"Error: Element with key "<<'"'<<dbjson[i].key()<<'"'<<" already exists!!"<<endl;
                return CMD_EXEC_DONE;
               }
              dbjson.add(DBJsonElem(options[0],buffer));
              return CMD_EXEC_DONE;
          }
          else 
...
{  
  ...
  else 
  {
   ...
   if (!CmdExec::lexSingleOption(option, token))
      return CMD_EXEC_ERROR;
 if (token.size()) //印出指定的那ㄧ個
{  int pos=dbjson.size();
   
  for(int i=0;i<dbjson.size();i++)
   if(token==dbjson[i].key()) pos=i;

   if(pos<dbjson.size()) cout<<"{ "<<dbjson[pos]<<" }"<<endl;
   else cerr<<"Error: No JSON element with key "<<'"'<<token<<'"'<<" is found."<<endl;
